...
HARD_COPY_DIR="${HARD_COPY_DIR:-${SERVER_DIR}/min-debs-hard-copy}"
...
if [[ -x "${SERVER_DIR}/make-local-repo.sh" ]]; then
	bash "${SERVER_DIR}/make-local-repo.sh"
else
...
echo "=== Step 2: Building server image OFFLINE (no network, no cache) ==="
cd "${SERVER_DIR}"
...
if ! sudo k3s crictl inspecti "${IMAGE_NAME}" >/dev/null 2>&1; then
	...
	if [[ -z "${IMPORTED_REF}" ]]; then
		...
		echo "Known containerd images matching 'samba|local':"
		sudo k3s ctr -n k8s.io images ls | egrep 'samba|local' || true
		exit 1
	fi
